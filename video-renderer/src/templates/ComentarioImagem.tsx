...
// Remove pontuação das bordas de um token para comparação,
// sem quebrar caracteres acentuados (ã, ç, é, etc.)
const stripPunctuation = (tok: string): string =>
  tok.replace(/^[\s.,!?;:()"'«»\-–—]+|[\s.,!?;:()"'«»\-–—]+$/g, '').toLowerCase();

// Constrói um Set com os índices de tokens que devem ser destacados.
// Estratégia dupla:
//   1. Palavras individuais: qualquer token cujo texto limpo esteja em
//      qualquer highlight é marcado — cobre "Fukuoka" mesmo que o highlight
//      seja "Buraco gigante" com palavras separadas no story.
//   2. Sequências completas: garante que os espaços *entre* palavras de um
//      highlight multi-token também fiquem dentro do intervalo marcado.
const buildHighlightedIndices = (tokens: string[], highlights: string[]): Set<number> => {
  const indices = new Set<number>();
  if (highlights.length === 0) return indices;

  // Palavras de cada highlight, já em lowercase
  const hlPhrases = highlights.map(hl =>
    hl.trim().split(/\s+/).map(t => t.toLowerCase())
  );

  // Conjunto plano de todas as palavras individuais dos highlights
  const hlWordSet = new Set(hlPhrases.flat());

  // Array só com os tokens de palavra (sem espaços), mantendo índice original
  const wordEntries = tokens
    .map((tok, i) => ({ tok, i }))
    .filter(({ tok }) => tok.trim().length > 0);

  // 1. Marca palavras individuais
  for (const { tok, i } of wordEntries) {
    if (hlWordSet.has(stripPunctuation(tok))) {
      indices.add(i);
    }
  }

  // 2. Marca sequências completas (inclui espaços entre as palavras)
  for (const hlTokens of hlPhrases) {
    if (hlTokens.length < 2) continue; // já coberto acima
...
// ── Texto estático com highlight (usado no hook) ───────────────────────────────
...
// ── Typewriter palavra por palavra com highlight (usado no story) ──────────────
...
const ProgressBar: React.FC = () => {
  ...
  return (
    <div style={{
      ...
      height: 6, backgroundColor: 'rgba(255,255,255,0.15)', zIndex: 100,
    }}>
      <div style={{
        height: '100%',
